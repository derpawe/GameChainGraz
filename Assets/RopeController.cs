...
public class RopeController : MonoBehaviour {
	void FixedUpdate() {
		...
		float radius = distance * distance / transform.childCount * 0.1f;
		...
		foreach (Transform child in transform) {
			...
			if (prevChild != null) {
				// break rope
				if ((child.position - prevChild.position).magnitude > 2.6) {
		...
		if (climber.position.y > jumper.position.y) {
			...
			if (distance > 13) {
				...
				climber.rigidbody2D.AddForce(new Vector2(0, distance * 0.01f));
				climber.Translate(new Vector2(0, distance * 0.01f));
			}
		}
	}
}
